...
public class DisplayMovies extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setCustomView(R.layout.dis_action_bar);
                ...
                while (cursor.moveToNext()) {
                    for (int j = 0; j < title_array.size(); j++) {
                        if (cursor.getString(1).equals(title_array.get(j))) {
                            ...
                            boolean isSelected = database.updateFav(title, year, director, actor, rating, review,is_fav);
